...
public class KMLFileWriter {
	...
	public void writeTrackPoints(FileWriter fw, GPXPath gpxPath) throws IOException {
		...
		for (int i = 1; i < points.size(); i++) {
			...
			String coordm1 = df.format(pm1.getLon()) + "," + df.format(pm1.getLat()) + "," + df.format(pm1.getZ());
			String coord = df.format(p.getLon()) + "," + df.format(p.getLat()) + "," + df.format(p.getZ());
	...
	private void writeTrackPoint(FileWriter fw, GPXPath gpxPath, int i, Point p, String coordm1, String coord,
		...
		fw.write("<TimeSpan>\n");
		if (delta1 == delta2) {
			fw.write("<begin>" + print(p.getTime() - delta1) + "</begin>\n");
		} else {
			fw.write("<begin>" + print(p.getTime() - delta1) + "</begin>\n");
			fw.write("<end>" + print(p.getTime() - delta2) + "</end>\n");
		}
		fw.write("</TimeSpan>\n");
		fw.write("      <LineString>\n");
	...
	private String print(long l) {
		return DateTimeFormatter.ISO_DATE_TIME.format(Instant.ofEpochMilli(l).atOffset(ZoneOffset.UTC));
	}
